...
- 팀 협업, 실시간 공유, 권한 관리
- 웹 UI / 모바일 앱
- 고도의 스케줄 최적화 알고리즘군 (유전/몬테카를로 등) — core는 단순 greedy 1종까지, 그 이상은 플러그인 영역
- 상시 데몬 / 서버 프로세스
...
## 5. 개발 원칙: 테스트 커버리지 100%

tasq는 개발 자체를 AI 에이전트 주도로 진행하는 것을 전제로 한다. 에이전트가 작성한 코드의 정확성을 사람의 정독에 의존할 수 없으므로, 그 보증은 테스트가 대신한다. AI가 테스트 작성 비용을 크게 낮추기 때문에 100%는 비현실적인 구호가 아니라 실행 가능한 기준이다.

- **테스트 커버리지 100%를 기준으로 한다.** 테스트 없는 코드는 머지되지 않는다.
- **CI는 커버리지 100% 미만이면 실패한다.** `bun test --coverage` + `bunfig.toml`의 coverageThreshold로 강제하고, GitHub Actions 필수 체크로 건다.
- 실행 불가능한 라인(프로세스 종료 핸들러 등)은 명시적인 ignore 주석으로만 제외할 수 있고, 제외 자체가 리뷰 대상이다.
- plugin-sdk는 공개 API이므로 단위 테스트에 더해 계약(contract) 테스트를 갖춘다.
...
| 테이블 | 내용 |
|---|---|
| `tasks` | id, title, body(markdown), status, priority, tags, project, parent_id(sub-task 트리), start(계획 시작), due, wait, until, recur, estimated_duration, external_ref, archived_at(soft delete), timestamps |
| `events` | append-only 이벤트 로그 — task_id, type, payload(JSON), ts. 상태 전이·블록·세션 기록 전부 |
| `task_deps` | 태스크 간 의존성 (task_id → depends_on_id). 순환 차단 |
| `agent_sessions` | task_id, adapter, mux 세션명, status(running/waiting_input/done/failed), 결과 요약, timestamps |
| `uda_defs` / `task_udas` | 사용자 정의 필드 정의와 값 |
| `contexts` / `app_config` | 작업 컨텍스트 필터와 앱 상태 |
...
- 태스크 상태 머신: `todo → in_progress → review → done` + `blocked` / `cancelled` (+ in_progress의 위임 변형 `delegated`). deps에 의한 블록은 status가 아닌 계산값.

상세 설계는 [core 기능 보강 설계](./2026-06-07-core-enhancement.md) 참조.
...
| 1.5 | core 보강 3배치: 사용성(날짜·deps·sub-task·soft delete) / 신뢰성(urgency·undo·recurrence) / 고급(wait·context·UDA·스케줄링) — [상세 설계](./2026-06-07-core-enhancement.md) | taskwarrior 수준의 일상 사용성을 갖췄는가 |
